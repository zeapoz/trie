...
impl<HO: std::fmt::Debug, CE: std::error::Error> std::fmt::Display for Error<HO, CE> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> Result<(), std::fmt::Error> {
        match self {
            ...
            Error::ExtraneousValue(key) => write!(
                f,
...
pub struct StackEntry<'a, L: TrieLayout> {
    /// The prefix is the nibble path to the node in the trie.
    pub prefix: LeftNibbleSlice<'a>,
    pub node: Node<'a>,
    pub is_inline: bool,
    ...
    pub children: Vec<Option<ChildReference<TrieHash<L>>>>,
    /// Technical to attach lifetime to entry.
    pub next_value_hash: Option<TrieHash<L>>,
...
impl<'a, L: TrieLayout> StackEntry<'a, L> {
    ...
    pub fn encode_node(mut self) -> Result<Vec<u8>, Error<TrieHash<L>, CError<L>>> {
